...
def function(d1,d2):
    ...
    dig=datax[:,784]
    # m=dig.shape[0]
    d4=datax[np.ix_(dig==d1,)]
    d5=datax[np.ix_(dig==d2,)]
    print(d4.shape)
    d4=np.delete(d4,784,1)
    print(d4.shape)
    np.savetxt("a.csv",np.array([1,2,3]))
    ...
    print(b)
    ww=np.concatenate((alpha,np.array(b).reshape((1,1))),axis=0)
    fn="d"+str(d1)+str(d2)+".csv"
    np.savetxt(fn,ww)
